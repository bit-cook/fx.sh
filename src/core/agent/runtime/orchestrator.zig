    ...
    while (agent_steps.allowsStep(config.agent_step_limit, step)) : (step += 1) {
        ...
        while (true) {
            ...
            const model_tools = try tool_advertisement.buildModelTools(
                overlay_arena,
                if (final_permission_response) &.{} else config.model_tools,
                if (final_permission_response) &.{} else selected_dynamic_tools.items,
                vision_mode,
            ...
            const model_request = agent_stream_provider.ModelRequest{
                .tools = model_tools,
                .provider_tools = if (final_permission_response or vision_mode == .required)
                    &.{}
                else
